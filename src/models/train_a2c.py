...
def main():
    ...
    p.add_argument("--save-dir", type=str, default="artifacts/a2c_model")
    args = p.parse_args()
    ...
    use_sde = args.action_mode in ("target","delta")
    model = A2C("MlpPolicy", env, learning_rate=args.learning_rate, ent_coef=args.ent_coef,
                use_sde=use_sde, sde_sample_freq=4, verbose=1).learn(total_timesteps=args.total_timesteps)
