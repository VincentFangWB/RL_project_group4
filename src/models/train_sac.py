...
def _parse_ent_coef(v: str) -> float | str:
    s=str(v).strip().lower()
    if s.startswith("auto"): return v
    try: return float(v)
    except: raise ValueError("--ent-coef must be float or 'auto'/'auto_X'")
...
def main():
    ...
    p.add_argument("--save-dir", type=str, default="artifacts/sac_model")
    args = p.parse_args()

    if args.action_mode == "discrete3":
        raise SystemExit("SAC requires a continuous action space; use --action-mode target or delta.")
    ...
    model = SAC("MlpPolicy", env, learning_rate=args.learning_rate,
                ent_coef=ent_coef, verbose=1).learn(total_timesteps=args.total_timesteps)
